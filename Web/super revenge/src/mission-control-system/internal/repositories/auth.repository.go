...
func (r *AuthRepository) IsAdmin(ctx context.Context, userID uint) (bool, error) {
...
func (r *AuthRepository) LoginUser(ctx context.Context, loginUserDto dtos.LoginUserDto) (dtos.StructuredResponse, error) {
	...
	if err := r.DB.WithContext(ctx).Preload("Role").Preload("Notes").Where("email = ?", loginUserDto.Email).First(&user).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return dtos.StructuredResponse{
				Success: false,
