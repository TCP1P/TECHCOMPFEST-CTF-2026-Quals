...
// @Accept json
// @Produce json
// @Param token body dtos.TokenDto true "JWT token to be validated"
// @Success 200 {object} dtos.StructuredResponse "Token is valid"
// @Failure 401 {object} dtos.StructuredResponse "Invalid token"
// @Failure 500 {object} dtos.StructuredResponse "Internal server error"
// @Router /auth/check-token [post]
func (h *AuthHandler) CheckToken(w http.ResponseWriter, r *http.Request) {
	h.Logger.Info("Check token request received")

	var req dtos.TokenDto

	if !h.DecodeJSONBody(w, r, &req) {
		return
	}

	h.Logger.Debug("Checking token", zap.String("token", req.Token))

	response, err := h.service.CheckToken(r.Context(), req)

	if err != nil {
		h.Logger.Error("Failed to check token", zap.Error(err))
		h.ReturnJSONResponse(w, dtos.StructuredResponse{
			Success: false,
			Status:  http.StatusInternalServerError,
			Message: err.Error(),
